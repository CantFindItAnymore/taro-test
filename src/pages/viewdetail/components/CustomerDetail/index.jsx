    ...
    <View className={styles.container}>
      ...
      >
        ...
        <AtTabsPane current={current} index={3}>
          {current === 3 && (
            <AtList>
              <AtListItem
                title="事业版块"
                note={data[current].value?.plateCode}
              />
              <AtListItem
                title="负责人"
                note={data[current].value?.principalUserName}
              />
              <AtListItem
                title="负责人部门"
                note={data[current].value?.principalOrganizationName}
              />
              <AtListItem
                title="销售"
                note={data[current].value?.salerUserName}
              />
            </AtList>
          )}
        </AtTabsPane>
        <AtTabsPane current={current} index={4}>
          {current === 4 && (
            <AtList>
              <AtListItem
                title="客户评分"
                note={data[current].value?.customerScore}
              />
              <AtListItem
                title="客户评级"
                note={data[current].value?.customerGrade}
              />
              <AtListItem
                title="授信额度上限"
                note={data[current].value?.creditLimit
                  ?.toString()
                  ?.replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
              />
              <AtListItem
                title="共享集团授信"
                note={data[current].value?.useCustomerCredit ? "是" : "否"}
              />
              <AtListItem
                title="申请授信额度"
                note={data[current].value?.applyCredit
                  ?.toString()
                  ?.replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
              />
              <AtListItem
                title="资格认证方式"
                note={data[current].value?.validationMethodName}
              />
              <AtListItem
                title="担保证明"
                note={
                  <ImageBox
                    imgs={data[current].value?.guaranteeProofUrls?.split(",")}
                  />
                }
              />
              <AtListItem
                title="剩余授信额度"
                note={data[current].value?.remainCredit
                  ?.toString()
                  .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
              />
            </AtList>
        ...
        <AtTabsPane current={current} index={5}>
          {current === 5 &&
            data[current].value?.map(_ => {
              return (
                <View key={_.id} className={styles.item}>
                  <Text>媒体:{_.mediaName}</Text>
                  <Text>媒体版块:{_.mediaSectionName}</Text>
                  <Text>业务类型:{_.businessTypeName}</Text>
                  <Text>客户账期:{_.customerPaymentValue}</Text>
                  <Text>缓冲账期:{_.bufferPaymentValue}</Text>
                  <Text>
                    有效期:{_.startDate} - {_.endDate}
                  </Text>
                </View>
              );
            })}
          ...
          {current === 6 &&
            data[current].value?.map(_ => {
              return (
                <View key={_.id} className={styles.item}>
                  <Text>媒体:{_.mediaName}</Text>
                  <Text>运营方式:{_.operationName}</Text>
                  <Text>政策标签:{_.policyLableName}</Text>
                  <Text>返点:{`${_.rebate * 100}%`}</Text>
                  <Text>有效开始日期:{_.startDate}</Text>
                  <Text>有效结束日期:{_.endDate}</Text>
                  <Text>创建时间:{_.createdTime}</Text>
                </View>
              );
            })}
        </AtTabsPane>
        <AtTabsPane current={current} index={7}>
          {current === 7 && (
            <AtList>
              <AtListItem
                title="资金来源"
                note={data[current].value?.capitalSourceName}
              />
              <AtListItem
                title="公司注册资本（实缴）"
                note={data[current].value?.registeredCapitalName
                  ?.toString()
                  .replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
              />
              <AtListItem
                title="公司组织形式"
                note={data[current].value?.organizationFormsName}
              />
              <AtListItem
                title="客户集团核实结果"
                note={data[current].value?.customerCapitalOpinion}
              />
              <AtListItem
                title="和我公司有过纠纷和诉讼"
                note={data[current].value?.legalDisputeName}
              />
              <AtListItem
                title="是否有外部高风险诉讼"
                note={data[current].value?.highRiskName}
              />
              <AtListItem
                title="集团信息确认说明"
                note={data[current].value?.customerInfoConfirmOpinion}
              />
              <AtListItem
                title="审批意见"
                note={data[current].value?.approvalOpinion}
              />
            </AtList>
          )}
        </AtTabsPane>
